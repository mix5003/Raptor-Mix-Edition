...
namespace NClass.CSharp
{
	...
	internal sealed class CSharpProperty : Property
	{
			...
			set
			{
				...
				if (match.Success) {
					...
					try {
						...
						else if (HasParameter) { // Not indexer
							ArgumentList.Clear();
						}
					}
					finally {
		...
		public override string Type
		{
			...
			{
				if (value == "void")
					throw new BadSyntaxException(Strings.GetString("error_type", "void"));
			...
			set
			{
				if (value && IsIndexer) {
					throw new BadSyntaxException(
						Strings.GetString("error_static_indexer"));
				}
				base.IsStatic = value;
			}
		}
		...
		public override AccessModifier AccessModifier
		{
			...
			set
			{
				if (value != AccessModifier.Default && IsExplicitImplementation) {
					throw new BadSyntaxException(
						Strings.GetString("error_explicit_implementation_access"));
				}
			...
			private set
			{
				if (isExplicitImplementation != value) {
					if (value && !(Parent is IInterfaceImplementer))
						throw new BadSyntaxException(Strings.GetString("error_explicit_implementation"));
