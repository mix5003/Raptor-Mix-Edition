...
namespace NClass.AssemblyImport
{
  ...
  /// get set up which hold some relations.
  /// 
  /// In the second pass all the relations in the dictionaries get added.
  /// </remarks>
...
//         AppDomain.Unload(xNewAppDomain);
        ...
        CreateNestingRelations();
        CreateGeneralizationRelations();
        CreateRealizationRelation();
      }
...
//       }
    ...
    #region +++ Pass 2 - Relations

    /// <summary>
    /// Creates all nested relations. Uses the inforamtion stored in the nestings
    /// dictionary which is filled in the first pass.
    /// </summary>
    private void CreateNestingRelations()
    {
      foreach(string stType in nestings.Keys)
      {
        if(entities.ContainsKey(nestings[stType]))
        {
          xProject.AddNestingRelation((CompositeType) entities[nestings[stType]],
    ...
    /// <summary>
    /// Creates all generalization relations. Uses information stored in the
    /// generalizations dictionary which is filled in the first pass.
    /// </summary>
    private void CreateGeneralizationRelations()
    {
        ...
        if(generalizations.ContainsKey(stChildTypeName))
        {
          foreach(string stBaseType in generalizations[stChildTypeName])
          {
            if(!string.IsNullOrEmpty(stBaseType) && entities.ContainsKey(stBaseType))
            {
              ...
              {
                xProject.AddGeneralizationRelation((CompositeType) entities[stChildTypeName],
    ...
    /// <summary>
    /// Creates all realization relations. Uses information stored in the 
    /// implementations dictionary which is filled in the first pass.
    /// </summary>
    private void CreateRealizationRelation()
    {
        ...
        if(entities.ContainsKey(stType))
        {
          foreach(string stInterface in implementations[stType])
          {
            if(entities.ContainsKey(stInterface))
            {
              ...
              {
                xProject.AddRealizationRelation((TypeBase) entities[stType],
    ...
    private void ReflectTypeBase(Type xType, TypeBase xTypeBase)
    {
      ...
      //corrected when adding the nesting relations.
