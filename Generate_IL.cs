...
namespace raptor
{
    public class Generate_IL : GeneratorAda.Imperative_Interface
    {
        ...
        public Generate_IL(string name)
        {
            ...
            //AppDomain new_domain = AppDomain.CreateDomain("newDomain");
            this.myAssemblyBuilder = Thread.GetDomain().DefineDynamicAssembly(myAssemblyName, AssemblyBuilderAccess.RunAndSave);
            // Create a dynamic module in the assembly.
            ModuleBuilder myModuleBuilder = myAssemblyBuilder.DefineDynamicModule("MyModule", name + ".exe");
        ...
        public void Emit_Assign_To(string name)
        {
            ...
            {
                local = gen.DeclareLocal(numbers_pkg.pi.GetType());
        ...
        public void Declare_As_Variable(string name)
        {
            if (!Previously_Declared(name))
            {
                System.Type t1 = numbers_pkg.pi.GetType();
