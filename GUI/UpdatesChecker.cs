	...
	public static class UpdatesChecker
	{
		...
		private class VersionInfo
		{
			...
			public bool IsTranslationUpdated
			{
				get
				{
					string currentTranslationVersion = Strings.GetString("translation_version");
		...
		private static VersionInfo GetVersionManifestInfo()
		{
			try {
				...
				XmlNodeList translationElements = root.SelectNodes(
					"TranslationVersions/" + Strings.GetString("translation_name")); //TODO: nem kéne listát használni!
				string translationVersion;
				if (translationElements.Count == 0)
					translationVersion = Strings.GetString("translation_version");
				else
		...
		public static void CheckForUpdates()
		{
			try {
				...
				MessageBox.Show(Strings.GetString("error_connect_to_server"),
					Strings.GetString("error"), MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
			catch (InvalidDataException) {
				MessageBox.Show(Strings.GetString("error_read_version_data"),
					Strings.GetString("error"), MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
		}
		...
		private static void ShowNewVersionInfo(VersionInfo info)
		{
			if (info.IsUpdated) {
				...
				string caption = Strings.GetString("checking_for_updates");
				...
				MessageBox.Show(
					Strings.GetString("no_updates"), Strings.GetString("checking_for_updates"),
		...
		private static string GetVersionDescription(VersionInfo info)
		{
			...
			if (info.IsMainProgramUpdated) {
				...
				builder.AppendFormat("{0}: {1}\n\n",
					Strings.GetString("new_version"), info.VersionName);
				...
				builder.AppendFormat("{0}\n\n", Strings.GetString("translation_updated"));
			}

			// Download text
			builder.AppendFormat(Strings.GetString("program_download"));
