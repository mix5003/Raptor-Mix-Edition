...
namespace NClass.Core
{
	public abstract class Method : Operation
	{
        ...
        public void getParameters(out int num_params,
            out string[] param_names,
            out bool[] param_is_input,
            out bool[] param_is_output) 
        {
            num_params = ArgumentList.Count;
            param_names = new String[num_params];
            param_is_input = new bool[num_params];
            param_is_output = new bool[num_params];
            for (int i = 0; i < num_params; i++)
            {
                param_is_input[i] = true;
                param_is_output[i] = false;
                param_names[i] = ArgumentList[i].Name;
            }
